...
def demo_bplustree():
    ...
    eng_bang_dictionary=open("English2BanglaDictionary.txt",encoding='utf-8-sig') #to avoid BOM=\ufeff character
    datasize=int(eng_bang_dictionary.readline())
    print(datasize)
    print( '\nInserting key from file...')
    for i in range(datasize):
        english=eng_bang_dictionary.readline().strip('\n')
        bangla=eng_bang_dictionary.readline().strip('\n')
        bplustree.insert(english, bangla)
    bplustree.show()

    #letter=input("Enter the word to be searched: ")
    print ('\nRetrieving values with key zoo...')
    print (bplustree.retrieve('zoo'))
